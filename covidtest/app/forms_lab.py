...
class LabCheckInForm(forms.Form):
    barcodes = forms.CharField(label=_('Barcodes'), max_length=100,
                              widget=forms.Textarea(attrs={'autofocus': 'autofocus', 'placeholder': 'Barcode ...'}))
    rack = forms.CharField(label=_('Rack (Barcode)'), max_length=100, widget=forms.TextInput(attrs={'placeholder': 'Rack ...'}))
